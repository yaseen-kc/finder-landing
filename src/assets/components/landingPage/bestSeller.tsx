...
export default function BestSeller() {
  ...
  // Map Best Seller IDs to Product Detail route IDs
  const toProductRouteId = (bestId: string): string => {
    const normalized = bestId.replace(/^finder-/, "");
    if (normalized === "wrist-binder-men-women") return "wrist-binder";
    return normalized;
  };

  return (
    <section className="mx-auto max-w-screen-2xl px-4 sm:px-6 lg:px-36 py-12">
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {items.map((item, index) => {
          const routeId = toProductRouteId(item.id);
          const href = `/product/${routeId}`;
          return (
            <article
              key={item.id}
              className={
            ...
            >
              ...
              <div className="px-4 pb-4">
                <button
                  type="button"
                  className="mt-2 w-full rounded-xl bg-accent-600 hover:bg-accent-700 text-white font-semibold py-3 transition-colors"
                >
